...
exports.MedicineGroupController = void 0;
...
class MedicineGroupController {
    ...
    async getAll(req, res) {
        try {
            ...
            const groups = await groupMedicine_1.GroupMedicine.find(query).lean();
            const enrichedGroups = await Promise.all(groups.map(group => this.enrichGroup(group)));
    ...
    async getById(req, res) {
        try {
            const group = await groupMedicine_1.GroupMedicine.findById(req.params.id).lean();
            if (!group)
    ...
    async create(req, res) {
        try {
            ...
            const group = await groupMedicine_1.GroupMedicine.create({ group_name, deleted_at });
    ...
    async update(req, res) {
        try {
            const group = await groupMedicine_1.GroupMedicine.findById(req.params.id);
            if (!group)
                return res.status(404).json({ error: 'Medicine group not found' });
            const group_name = req.body.group_name ?? req.body.title;
            const deleted_at = req.body.deleted_at;
...
exports.MedicineGroupController = MedicineGroupController;
